...
object UI:
  ...
  private def renderPitchSlot(slot: LineupSlot, team: Team): HTMLElement =
    ...
    val label = document.createElement("div").asInstanceOf[HTMLElement]
    label.className = "mf-pitch-slot-label"
    label.textContent = slot.expectedPosition.toString
    elem.appendChild(label)

    val playerSpan = document.createElement("div").asInstanceOf[HTMLElement]
    playerSpan.className = "mf-pitch-slot-player"

    val maybePlayer = slot.player.flatMap(pid => team.players.find(_.id == pid))

    val button = document.createElement("button").asInstanceOf[HTMLElement]
    button.className = "mf-pitch-slot-btn"

    if maybePlayer.isEmpty then
